...
def _tag_rds(rid, tags):
    ...
    # Fetch ARN first — add_tags_to_resource requires the ARN, not DB id.
...
def _tag_s3(rid, tags):
    s3 = boto3.client("s3")
    # MUST merge with existing tags — put_bucket_tagging replaces entire set.
    try:
        existing = s3.get_bucket_tagging(Bucket=rid).get("TagSet", [])
    except ClientError:
        existing = []

    # Build merged dict: existing first, then new tags overwrite same keys.
